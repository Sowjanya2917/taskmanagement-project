...
import { processRecurringTasks, updateDependencyStatuses } from '../utils/RecurringTaskService';
...
const Dashboard = () => {
  ...
  // Fetch team data and initial task processing
  useEffect(() => {
    if (!currentUser) return;

    // Check task deadlines for notifications
    checkTaskDeadlines();
    
    // Process recurring tasks and update dependency statuses when dashboard loads
    const initializeTaskProcessing = async () => {
      try {
        // Process any pending recurring tasks
        await processRecurringTasks();
        
        // Update dependency statuses for all tasks
        await updateDependencyStatuses();
      } catch (error) {
        console.error("Error initializing task processing:", error);
      }
    };
    
    initializeTaskProcessing();
    
    // Set up intervals for recurring tasks and deadline notifications
    ...
    return () => {
      clearInterval(recurringTasksInterval);
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        taskList.push({
          ...
          // Make sure dependency status is tracked
          dependenciesMet: taskData.dependenciesMet !== undefined ? taskData.dependenciesMet : true
        });
      });
  ...
  const addTask = async (task) => {
    try {
      const taskData = {
        ...
        dependenciesMet: !task.dependencies || task.dependencies.length === 0 // Set initial dependency status
      };
  ...
  const updateTask = async (taskId, updatedData) => {
    try {
      ...
      // If dependencies are updated, check their completion status
      if (updatedData.dependencies !== undefined) {
        const dependenciesMet = !updatedData.dependencies.length || 
          updatedData.dependencies.every(dep => dep.status === 'completed');
        
        updatedData.dependenciesMet = dependenciesMet;
      }
  ...
  // Update task status with support for dependencies and recurrence
  const updateTaskStatus = async (taskId, newStatus) => {
    try {
      ...
      // Check if we're completing a task that has dependencies
      if (newStatus === 'completed' && task.status !== 'completed') {
        // Check if all dependencies are completed
        if (task.dependencies && task.dependencies.length > 0 && !task.dependenciesMet) {
          alert("Cannot complete this task. Not all dependencies have been completed.");
          return false;
        }
        ...
        if (task.recurrence && task.recurrence.isRecurring) {
          const { createNextRecurrence } = await import('../utils/RecurringTaskService');
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    // Get task to check dependencies if completing
    if (targetStatus === 'completed') {
      const task = tasks.find(t => t.id === taskId);
      if (task && task.dependencies && task.dependencies.length > 0 && !task.dependenciesMet) {
        alert("Cannot complete this task. Not all dependencies have been completed.");
        return;
      }
    }
